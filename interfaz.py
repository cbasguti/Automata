...
def iniciar_programa(root, afnd):
    ...
    automata.setEstados(lista_estados)
    automata.setEstadoInicial(estado_inicial)
    automata.setEstadoFinal(lista_aceptacion)
    automata.setSimbolos(lista_simbolos)
    automata.setTransiciones(transiciones)
    if afnd:
        automata.convierte_afnd()
    else:
        automata.simplifica()
    auto_plugin = SimpleDFA(set(automata.estados), set(automata.simbolos), automata.estadoInicial, set(automata.estadoFinal), automata.transiciones)
...
# Importing a image
img = ImageTk.PhotoImage(Image.open("img/Automata.png"))
